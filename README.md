...
カジュアルモバイルゲームスイート向けの**統合スタイライズドシェーダーライブラリ**を構築するプロジェクトです。

### プロジェクトビジョン

**ポケコロ風キャラクターカスタマイズゲーム + パーティーミニゲーム集**

#### メインゲーム: キャラクターカスタマイズ＆デコレーション
- **参考**: ココネ「ポケコロ」または「リヴリーアイランド」スタイル
- **アセット**: SD Unity-Chan + SoStylized + AITranslator
- **機能**: アバター作成、着せ替え、部屋デコレーション、ソーシャル要素
- **ビジュアル**: トゥーンシェーディングキャラクター × スタイライズド環境

#### サブゲーム: クラシックボードゲーム集（ヘックスタイル版）
- **参考**: リバーシ、チェッカー、マンカラ等の古典ボードゲーム
- **アセット**: 8Set Hex Tiles（12タイル）+ Tomatocol Characters（20スプライト）
- **機能**: ターン制ヘックスボードゲーム、1-4プレイヤー対応、短時間プレイ（1-3分/ゲーム）
- **ビジュアル**: 2Dトップダウンビュー、シンプルスタイライズド（パフォーマンス優先）

**計画中のミニゲーム（10種）:**
- **Phase 1（MVP）**: Tic-Tac-Toe Hex、Hex Reversi、Hex Checkers
- **Phase 2**: Hex Chinese Checkers、Hex Mancala
- **Phase 3**: Hex Catan（簡易版）、Hex Battleship
...
**メインゲーム用（キャラクターカスタマイズ）:**
- **SD Unity-Chan Haon Custom** - Unity-Chan Toon Shader 2.0.6（トゥーンシェーディング参考実装）
- **SoStylized** - URP最適化スタイライズド環境アセット（背景、部屋デコレーション）
- **AITranslator** - 多言語対応ツール

**ミニゲーム用:**
- **8Set Free 2D Hex Tiles** - 12枚のヘックスタイル（4地形、2特殊、6装飾）
- **TomatocolCharacterVarietyPackVol1DEMO** - 20種の2Dキャラクタースプライト（ゲーム駒として使用）

### 第1マイルストーン: シェーダーテンプレートコレクション（3-4日）

**目的**: メインゲームとミニゲーム両方で使える統一シェーダーライブラリ基盤を構築

#### 成果物
1. **SG_Character_Base.shadergraph** - 肌/ボディ用基礎シェーダー
   - メインゲーム: カスタマイズアバター（近距離・詳細表示）
   - ミニゲーム: プレイヤーキャラクター（遠距離・簡略表示）

2. **SG_Character_Hair.shadergraph** - 髪専用レンダリング
   - メインゲーム: 高品質ヘアスタイル（異方性ハイライト、Angel Ring）
   - ミニゲーム: シンプルヘア（基本トゥーンのみ）

3. **SG_Character_Cloth.shadergraph** - 衣装/布表現
   - メインゲーム: 多層カスタマイズ衣装（4ゾーンカラーマスク、サテン/レース）
   - ミニゲーム: シンプル衣装（2ゾーンカラー）

4. **SG_Environment_Stylized.shadergraph** - スタイライズド環境
   - メインゲーム: 部屋デコレーション、背景（SoStylized品質）
   - ミニゲーム: ゲームステージ背景（ヘックスタイル対応）

#### シェーダー設計考慮事項（メインゲーム vs ミニゲーム）

**カメラアングル対応:**
- メインゲーム: 近距離カメラ（ディテール重視、アウトライン太め）
- ミニゲーム: 上空/俯瞰カメラ（視認性重視、アウトライン細め、明るいカラー）

**パフォーマンス階層:**
- メインゲーム: 1-2キャラクター表示、高品質シェーダー（マットキャップ、異方性有効）
- ミニゲーム: 複数キャラクター同時表示、軽量シェーダー（基本トゥーンのみ）

**カスタマイズ範囲:**
- メインゲーム: 全機能有効（4ゾーンカラー、テクスチャスワップ、レイヤリング）
- ミニゲーム: 基本カスタマイズ（2ゾーンカラー、プリセット選択）

**ボードゲーム固有の要件:**
- ヘックスタイル選択ハイライト（チームカラー対応）
- 有効手表示（半透明オーバーレイ）
- ゲーム駒アニメーション（配置、移動、キャプチャー）
- シンプルな2Dスプライトレンダリング

#### 各テンプレートの特徴
- クリーンなノード構造（コメント付きグループ化）
- プレースホルダーCustom Functionノード（適切に配線済み）
- **LOD/品質レベル対応**（メインゲーム高品質 ↔ ミニゲーム軽量）
